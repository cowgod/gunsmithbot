...
class GunsmithBot
  ...
  def query(requested_gamertag, requested_platform, requested_slot)
    ...
    raise ArgumentError, "Couldn't find the requested slot." unless bucket_id
    ...
    unless user_info
      ...
      raise ArgumentError, "Couldn't find the requested user." unless search_results&.first
    ...
    raise ArgumentError, "Couldn't find the requested user." unless user_info
    ...
    raise ArgumentError, "Couldn't find the most recently used character for the requested user." unless character

    requested_item = character.dig('items').find { |item| item.dig('bucketHash') == bucket_id }
    raise ArgumentError, "Couldn't find the requested item or armor piece." unless requested_item

    results[:item] = @bungie_api.item_details(user_info['membershipType'], user_info['membershipId'], requested_item['itemInstanceId'])
    raise ArgumentError, "Couldn't load info for the requested item or armor piece." unless results[:item]
